...
interface (`buffers_param`) to avoid templating the implementation on
buffer types.
...
== The buffers_param Interface

Internally, Corosio uses `buffers_param` to type-erase buffer sequences.
...
The `buffers_param_impl` adapter wraps any buffer sequence:

[source,cpp]
----
template<class Buffers>
class buffers_param_impl final : public buffers_param</*IsMutable*/>;
----
...
void socket_impl::read_some(buffers_param<true>& bufs, ...);
----
...
* xref:reference/tcp-endpoint.adoc[tcp::endpoint] — Endpoint reference
