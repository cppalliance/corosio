...
Simple, Unreliable]. This page covers the Corosio API: `udp_socket` and the
...
`open()` defaults to `family::v4` when no family is supplied, and
`connect()` picks the family automatically from the endpoint when the
