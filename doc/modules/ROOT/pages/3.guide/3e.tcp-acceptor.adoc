...
For production servers, consider using xref:3k.tcp-server.adoc[tcp_server] which
provides:
...
* xref:3d.sockets.adoc[Sockets] — Using accepted connections
* xref:3k.tcp-server.adoc[TCP Server] — Higher-level server framework
* xref:../2.tutorials/2a.echo-server.adoc[Echo Server Tutorial] — Complete example
