...
byte-level staging in xref:5a.mocket.adoc[`mocket`] cannot reproduce.
...
xref:5c.patterns.adoc[Testing Patterns].
...
* xref:5a.mocket.adoc[Mock Sockets] — for byte-level deterministic
  tests.
* xref:5c.patterns.adoc[Testing Patterns] — recipes that combine both.
