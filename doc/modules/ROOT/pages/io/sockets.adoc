...
[source,cpp]
----
corosio::tcp::endpoint ep(boost::urls::ipv4_address::loopback(), 8080);
    ...
    while (total < len)
    {
        ...
        if (n == 0)
            co_return {boost::system::errc::make_error_code(
                boost::system::errc::connection_reset), total};
...
* xref:io/acceptor.adoc[Acceptor] — Server-side connection acceptance
