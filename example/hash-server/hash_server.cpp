...
// tag::session[]
capy::task<>
do_session(corosio::tcp_socket sock, capy::thread_pool& pool)
{
    char buf[4096];
    ...
    // tag::run_switch[]
    auto hash = co_await capy::run(pool.get_executor())(compute_fnv1a(buf, n));
    // end::run_switch[]

    // Send hex result back to client (on io_context)
    auto result                     = to_hex(hash) + "\n";
    [[maybe_unused]] auto [wec, wn] = co_await capy::write(
        sock, capy::const_buffer(result.data(), result.size()));
    ...
    capy::thread_pool& pool)
{
    for (;;)
    {
        ...
        capy::run_async(ioc.get_executor())(do_session(std::move(peer), pool));
    }
}
// end::accept[]
