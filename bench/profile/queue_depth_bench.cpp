    ...
    while (std::chrono::steady_clock::now() < end_time)
    {
        ...
        if (now >= next_report)
        {
            ...
            std::cout << "  [" << std::fixed << std::setprecision(0) << elapsed << "s] "
                      << bench::format_rate(rate) << " (" << iterations << " iterations)\n";
    ...
    int num_threads)
{
    ...
    // Main workload
    run_workload<Context>(duration, queue_depth, num_threads);
...
int main(int argc, char* argv[])
{
    ...
    return bench::dispatch_backend(backend,
        ...
        {
            run_profiler_workload<Context>(name, duration, queue_depth, num_threads);
        });
}
