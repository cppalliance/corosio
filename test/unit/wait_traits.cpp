...
struct wait_traits_test
{
    ...
    {
        auto d = std::chrono::system_clock::duration(std::chrono::seconds(5));
        BOOST_TEST(
            wait_traits<std::chrono::system_clock>::to_wait_duration(d) == d);
        BOOST_TEST(
            wait_traits<minute_clock>::to_wait_duration(
    ...
    void testConcept()
    {
        ...
        static_assert(WaitTraits<wait_traits<minute_clock>, minute_clock>);
