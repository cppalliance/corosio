...
bool
completion_should_fail(unsigned long& err) noexcept
{
...
bool
completion_fault_scope::fired() const noexcept
{
...
#define COROSIO_FAULT_WIN_SIMPLE(X)                                            \
    ...
    X(bind, int, SOCKET_ERROR, WSAAPI, (SOCKET s, sockaddr const* a, int len), \
      (s, a, len))                                                             \
    ...
    X(CreateFileW, HANDLE, INVALID_HANDLE_VALUE, WINAPI,                       \
      (LPCWSTR name, DWORD access, DWORD share, LPSECURITY_ATTRIBUTES sa,      \
    ...
    X(SetFilePointerEx, BOOL, FALSE, WINAPI,                                   \
      ...
      (h, dist, out, method))                                                  \
    X(GetFileSizeEx, BOOL, FALSE, WINAPI, (HANDLE h, PLARGE_INTEGER size),     \
      (h, size))                                                               \
    ...
    X(SetWaitableTimer, BOOL, FALSE, WINAPI,                                   \
      (HANDLE h, LARGE_INTEGER const* due, LONG period, PTIMERAPCROUTINE apc,  \
       LPVOID arg, BOOL resume),                                               \
      (h, due, period, apc, arg, resume))                                      \
    X(WaitForSingleObject, DWORD, WAIT_FAILED, WINAPI, (HANDLE h, DWORD ms),   \
      (h, ms))                                                                 \
...
#define COROSIO_FAULT_WIN_OPENSSL(X)                                        \
    X(BIO_new_mem_buf, void*, nullptr, __cdecl, (void const* buf, int len), \
      (buf, len))                                                           \
    ...
    X(X509_VERIFY_PARAM_set1_host, int, 0, __cdecl,                         \
      (void* p, char const* name, std::size_t namelen), (p, name, namelen))
...
enum hook_id
{
    COROSIO_FAULT_WIN_SIMPLE(COROSIO_FAULT_WIN_ID)
        COROSIO_FAULT_WIN_OPENSSL(COROSIO_FAULT_WIN_ID)
            COROSIO_FAULT_WIN_MANUAL(COROSIO_FAULT_WIN_ID1) h_beginthreadex,
    hook_count
};
...
DWORD
truncate_wsabuf(
    WSABUF const* in, DWORD n, std::size_t count, WSABUF* out) noexcept
{
    if (n > 64u)
...
int WSAAPI
hooked_recv(SOCKET s, char* buf, int len, int flags)
{
    if (should_fail(sys::recv))
        return SOCKET_ERROR;
    auto const real =
        COROSIO_FAULT_WIN_CALL(recv, int, WSAAPI, (SOCKET, char*, int, int));
    std::size_t c = 0;
    if (should_shorten(sys::recv, c))
    {
        if (c == 0)
            return 0;
        return real(
            s, buf, static_cast<int>(c) < len ? static_cast<int>(c) : len,
            flags);
    }
...
int WSAAPI
hooked_send(SOCKET s, char const* buf, int len, int flags)
{
    if (should_fail(sys::send))
        return SOCKET_ERROR;
    auto const real = COROSIO_FAULT_WIN_CALL(
        send, int, WSAAPI, (SOCKET, char const*, int, int));
    std::size_t c = 0;
...
BOOL WINAPI
hooked_ReadFile(HANDLE h, LPVOID buf, DWORD len, LPDWORD got, LPOVERLAPPED ov)
{
    if (should_fail(sys::ReadFile))
        return FALSE;
    auto const real = COROSIO_FAULT_WIN_CALL(
        ReadFile, BOOL, WINAPI, (HANDLE, LPVOID, DWORD, LPDWORD, LPOVERLAPPED));
    std::size_t c = 0;
    if (should_shorten(sys::ReadFile, c))
        return real(
            h, buf, static_cast<DWORD>(c) < len ? static_cast<DWORD>(c) : len,
            got, ov);
    return real(h, buf, len, got, ov);
}

BOOL WINAPI
hooked_WriteFile(HANDLE h, LPCVOID buf, DWORD len, LPDWORD put, LPOVERLAPPED ov)
{
...
void WSAAPI
hooked_FreeAddrInfoExW(PADDRINFOEXW ai)
{
...
sig_handler_t __cdecl
hooked_signal(int sig, sig_handler_t handler)
{
    if (should_fail(sys::signal))
        return SIG_ERR;
    return COROSIO_FAULT_WIN_CALL(
        signal, sig_handler_t, __cdecl, (int, sig_handler_t))(sig, handler);
}
    ...
    LPOVERLAPPED ov)
{
    if (should_fail(sys::AcceptEx))
        return FALSE;
    return real_accept_ex(
        listener, accepted, buf, recv_len, local_len, remote_len, got, ov);
}
...
LONG NTAPI
hooked_NtSetInformationFile(
    HANDLE h, ULONG_PTR* iosb, void* info, ULONG len, ULONG cls)
{
    if (should_fail(sys::NtSetInformationFile))
        return status_unsuccessful;
    return reinterpret_cast<LONG(NTAPI*)(
        HANDLE, ULONG_PTR*, void*, ULONG, ULONG)>(real_nt_set_information_file)(
        h, iosb, info, len, cls);
}

LONG NTAPI
hooked_NtFlushBuffersFileEx(
    HANDLE h, ULONG flags, void* params, ULONG len, void* iosb)
{
    if (should_fail(sys::NtFlushBuffersFileEx))
        return status_unsuccessful;
    return reinterpret_cast<LONG(NTAPI*)(HANDLE, ULONG, void*, ULONG, void*)>(
        real_nt_flush_buffers_file_ex)(h, flags, params, len, iosb);
}

bool
same_guid(void const* lhs, GUID const& rhs) noexcept
{
    ...
    LPWSAOVERLAPPED_COMPLETION_ROUTINE cr)
{
    if (should_fail(sys::WSAIoctl))
        return SOCKET_ERROR;
    int const r = COROSIO_FAULT_WIN_CALL(
        WSAIoctl, int, WSAAPI,
        (SOCKET, DWORD, LPVOID, DWORD, LPVOID, DWORD, LPDWORD, LPWSAOVERLAPPED,
         LPWSAOVERLAPPED_COMPLETION_ROUTINE))(
        s, code, in, inlen, out, outlen, ret, ov, cr);
    if (r != 0 || code != SIO_GET_EXTENSION_FUNCTION_POINTER || !in ||
        inlen < sizeof(GUID) || !out || outlen < sizeof(void*))
        return r;
...
FARPROC WINAPI
hooked_GetProcAddress(HMODULE mod, LPCSTR name)
{
    if (should_fail(sys::GetProcAddress))
        return nullptr;
    FARPROC const p = COROSIO_FAULT_WIN_CALL(
        GetProcAddress, FARPROC, WINAPI, (HMODULE, LPCSTR))(mod, name);
...
BOOL WINAPI
hooked_GetQueuedCompletionStatus(
    HANDLE port, LPDWORD bytes, PULONG_PTR key, LPOVERLAPPED* ov, DWORD ms)
{
    ...
    BOOL const r = COROSIO_FAULT_WIN_CALL(
        GetQueuedCompletionStatus, BOOL, WINAPI,
...
std::uintptr_t WINAPIV
hooked_beginthreadex(
    void* security,
    unsigned stack,
    ...
    unsigned* id)
{
    ...
    return COROSIO_FAULT_WIN_CALL(
        beginthreadex, std::uintptr_t, WINAPIV,
...
hook_entry hooks[] = {
    COROSIO_FAULT_WIN_SIMPLE(COROSIO_FAULT_WIN_ROW) COROSIO_FAULT_WIN_OPENSSL(
        COROSIO_FAULT_WIN_ROW) COROSIO_FAULT_WIN_MANUAL(COROSIO_FAULT_WIN_ROW1){
        "_beginthreadex", sys::pthread_create,
        reinterpret_cast<proc_t>(&hooked_beginthreadex), 0},
};

static_assert(
    sizeof(hooks) / sizeof(hooks[0]) == hook_count,
...
hook_entry*
find_hook(char const* name) noexcept
{
    for (auto& h : hooks)
    {
...
char const*
module_name(HMODULE mod) noexcept
{
    static char buf[MAX_PATH + 1];
    if (!::GetModuleFileNameA(mod, buf, MAX_PATH))
        std::snprintf(
            buf, sizeof(buf), "<module %p>", reinterpret_cast<void*>(mod));
    return buf;
}
...
bool
name_is_ws2_32(char const* dll) noexcept
{
...
char const*
winsock_ordinal_name(char const* dll, unsigned ordinal) noexcept
{
...
template<class F>
void
for_each_import(HMODULE mod, F&& f) noexcept
{
         ...
         imp->Name != 0; ++imp)
    {
        ...
        if (imp->OriginalFirstThunk == 0)
            continue;
        char const* const dll = reinterpret_cast<char const*>(base + imp->Name);
        ...
        for (; names->u1.AddressOfData != 0; ++names, ++thunk)
        {
            ...
            {
                name = winsock_ordinal_name(
                    dll,
                    static_cast<unsigned>(IMAGE_ORDINAL(names->u1.Ordinal)));
            }
            else
            {
                auto const* by_name =
...
void
patch_module(HMODULE mod) noexcept
{
    for_each_import(mod, [](char const* name, IMAGE_THUNK_DATA& thunk) {
        ...
        DWORD old = 0;
        if (!::VirtualProtect(
                &thunk.u1.Function, sizeof(void*), PAGE_READWRITE, &old))
        {
            char msg[192];
            std::snprintf(
                msg, sizeof(msg),
...
void
patch_module_one(HMODULE mod, hook_entry& h) noexcept
{
    for_each_import(mod, [&](char const* name, IMAGE_THUNK_DATA& thunk) {
...
void
verify_module(HMODULE mod, bool& ok) noexcept
{
    for_each_import(mod, [&](char const* name, IMAGE_THUNK_DATA& thunk) {
        hook_entry* h = find_hook(name);
        if (!h)
        ...
        std::fprintf(
            stderr,
            "fault harness: %s in %s is bound to %p, hook "
            "is %p\n",
            name, module_name(mod), reinterpret_cast<void*>(thunk.u1.Function),
...
bool
name_holds_corosio(wchar_t const* name) noexcept
{
...
std::size_t
collect_modules(HMODULE* out, std::size_t cap) noexcept
{
    std::size_t n = 0;
    auto add      = [&](HMODULE m) {
        if (!m)
...
int const installed = [] {
    ...
    for (auto const& h : hooks)
    {
        ...
        std::fprintf(
            stderr, "fault harness: %s is imported by no corosio module\n",
            h.name);
    }
    if (!ok)
