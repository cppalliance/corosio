...
struct tls_session_cycle_stress_impl
{
    ...
    run()
    {
        ...
        std::fprintf( stderr,
            "  tls_session_cycle: running for %d seconds...\n", duration );
        ...
        std::fprintf( stderr,
            "  tls_session_cycle: %zu sessions completed\n", iterations );
...
struct tls_concurrent_io_stress_impl
{
    ...
    run()
    {
        int duration = get_tls_stress_duration();
        std::fprintf( stderr,
            "  tls_concurrent_io: running for %d seconds...\n", duration );
        ...
        std::fprintf( stderr,
            "  tls_concurrent_io: %zu total bytes transferred\n",
            total_bytes.load() );
...
struct tls_cancel_handshake_stress_impl
{
    ...
    run()
    {
        int duration = get_tls_stress_duration();
        std::fprintf( stderr,
            "  tls_cancel_handshake: running for %d seconds...\n", duration );
        ...
        while( std::chrono::steady_clock::now() < stop_time )
        {
            ...
            {
                char buf[1];
                (void)co_await s2.read_some(
                    capy::mutable_buffer( buf, 1 ) );
                stop_src.request_stop();
            };

            bool failsafe_hit = false;
            auto failsafe_task = [&failsafe, &failsafe_hit,
                &s1, &s2]() -> capy::task<>
            {
...
TEST_SUITE( openssl_session_cycle_stress,
...
TEST_SUITE( openssl_concurrent_io_stress,
...
TEST_SUITE( openssl_cancel_handshake_stress,
...
TEST_SUITE( wolfssl_session_cycle_stress,
...
TEST_SUITE( wolfssl_concurrent_io_stress,
...
TEST_SUITE( wolfssl_cancel_handshake_stress,
