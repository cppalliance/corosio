...
struct local_datagram_socket_test
{
    ...
    {
        // A zero-length datagram is a legitimate event for connected
        // datagram sockets and must NOT be reported as EOF (cond::eof).
        // Regression test for the reactor backends, where
        // reactor_dgram_recv_op previously inherited complete_io_op's
        // stream-style "is_read_operation && bytes==0 -> EOF" mapping.
        io_context ioc(Backend);
        auto [s1, s2] = make_local_datagram_pair(ioc);
